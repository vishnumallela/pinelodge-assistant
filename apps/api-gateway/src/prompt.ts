...
export function defaultTemplate(centerName: string): string {
  return [
    ...
    `- Before you look someone up or think for a beat, say a quick natural filler so the caller isn't met with silence — "Let me check…", "One sec…", "Let me pull that up." Then go quiet and do it. Don't overuse it.`,
...
function messageModePrompt(centerName: string, greeting: string): string {
  return [
    ...
    `- Before you pause to note something down, say a quick natural filler — "One sec…", "Let me get that down" — so the caller isn't met with silence.`,
